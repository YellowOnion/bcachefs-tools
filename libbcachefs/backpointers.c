...
found:
		...
		bch2_trans_iter_init(trans, &iter, BTREE_ID_backpointers,
				     backpointer_pos(c, alloc_pos, bp_offset - BACKPOINTER_OFFSET_MAX),
				     BTREE_ITER_INTENT|
				     BTREE_ITER_SLOTS|
				     BTREE_ITER_WITH_UPDATES);
		k = bch2_btree_iter_peek_slot(&iter);
			      ...
			      struct bch_backpointer *dst)
{
	...
	for_each_btree_key(trans, bp_iter, BTREE_ID_backpointers,
		...
		BUG_ON(k.k->p.offset - bucket_to_sector(ca, bucket) + BACKPOINTER_OFFSET_MAX < *bp_offset);
		*bp_offset = k.k->p.offset - bucket_to_sector(ca, bucket) + BACKPOINTER_OFFSET_MAX;
