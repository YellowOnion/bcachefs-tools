...
static struct open_bucket *try_alloc_bucket(struct btree_trans *trans, struct bch_dev *ca,
					    enum alloc_reserve reserve, u64 b, s64 genbits,
					    ...
					    struct closure *cl)
{
	...
	if (unlikely(ca->buckets_nouse && test_bit(b, ca->buckets_nouse)))
		return NULL;

	if (bch2_bucket_is_open(c, ca->dev_idx, b))
		return NULL;

	if (bch2_bucket_needs_journal_commit(&c->buckets_waiting_for_journal,
			c->journal.flushed_seq_ondisk, ca->dev_idx, b)) {
		(*need_journal_commit)++;
		return NULL;
	}

	if (bch2_fs_inconsistent_on(b < ca->mi.first_bucket ||
				    b >= ca->mi.nbuckets, c,
	...
	if (bch2_fs_inconsistent_on(genbits >= 0 && genbits != alloc_freespace_genbits(a), c,
			"bucket in freespace btree with wrong genbits (got %llu should be %llu)\n  %s",
						   ...
						   struct closure *cl)
{
	...
	if (likely(ca->mi.freespace_initialized)) {
