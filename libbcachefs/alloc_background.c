				...
				bool set)
{
	...
	if (ca->mi.freespace_initialized &&
	    bch2_fs_inconsistent_on(old.k->type != old_type, c,
			"inconsistency when updating %s btree: expected to find key type %s, got %s",
			  ...
			  unsigned flags)
{
	...
	if (new_u.dirty_sectors > old_u.dirty_sectors ||
		...
		new_u.read_time = atomic64_read(&c->io_clock[READ].now);
		new_u.write_time = atomic64_read(&c->io_clock[WRITE].now);
		need_repack = true;
	}
	...
	if (bucket_state(old_u) != bucket_state(new_u) ||
		...
		ret =   bch2_bucket_do_index(trans, old_u, false) ?:
			bch2_bucket_do_index(trans, new_u, true);
		if (ret)
			return ret;
	}
				...
				struct btree_iter *alloc_iter)
{
	...
	if (fsck_err_on(k.k->type != discard_key_type, c,
			"incorrect key in need_discard index (got %s should be %s)\n"
			"  %s",
			bch2_bkey_types[k.k->type],
			bch2_bkey_types[discard_key_type],
			(bch2_bkey_val_to_text(&PBUF(buf), c, alloc_k), buf))) {
				    ...
				    bool initial)
{
	...
	/*
	 * verify coresponding device exists and bucket is in within
	 * allowed range
	 */
...
static int bch2_dev_freespace_init(struct bch_fs *c, struct bch_dev *ca)
{
	...
	for_each_btree_key(&trans, iter, BTREE_ID_alloc,
		...
		ret = __bch2_trans_do(&trans, NULL, NULL,
				 ...
				 bch2_bucket_do_index(&trans, a, true));
		if (ret)
			break;
	}
	bch2_trans_iter_exit(&trans, &iter);
	...
	m = bch2_sb_get_members(ca->disk_sb.sb)->members + ca->dev_idx;
	SET_BCH_MEMBER_FREESPACE_INITIALIZED(m, true);
	ca->mi.freespace_initialized = true;
